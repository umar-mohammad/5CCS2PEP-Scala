...
def occurrences(xs: List[String]): Map[String, Int] = {
    xs.distinct // makes list of unique letters
    .map(letter => (letter , xs.count(_ == letter))) // creates list of tuples of the letter and how many times it occurs in the list
    .toMap // converts the list of tuples to map
}
...
def prod(lst1: List[String], lst2: List[String]) : Int = {
    ...
    val ocr1 = occurrences(lst1)
    val ocr2 = occurrences(lst2)

    val letters = (ocr1.keySet.toList:::ocr2.keySet.toList).distinct
    (for(x <- letters) yield ocr1.getOrElse(x, 0) * ocr2.getOrElse(x, 0))
    .sum
}
